...
int main() {
    ...
    int n,a[10],f[10],i,s[50],j,b[20],c[20],m;
	...
	cout<<"Enter the frequency of each element:"<<endl;
	for(i=0;i<n;i++)
    {
    	cin>>f[i];
	}
    ...
    {
		int k=(nos-1)/2;           ///k is index of median_q2-1.
  //      median_q2=a[(nos-1)/2];
        for(i=0;i<((nos-1)/2);i++)
        {
        	b[i]=s[i];
    ...
    else
    {
    	int k=nos/2;
    	cout<<"k= "<<k<<endl;
       int sl=s[(nos-1)/2]+s[nos/2];
	   float median_q2=sl/2.0;
		...
		else
		{
			cout<<"k=  "<<k<<endl;
			int first_s=b[(k-1)/2] + b[k/2];
			cout<<"b["<<(k-1)/2<<"]= "<<b[(k-1)/2]<<endl;
			cout<<"b[k/2]= "<<b[k/2]<<endl;
			cout<<"first_s= "<<first_s<<endl;
        	median_q1=first_s/2.0;
